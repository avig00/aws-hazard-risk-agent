...
IMPORTANT: If the question names a specific county (e.g. "What is the risk score for Harris County"), do NOT call that county "the highest-ranked" unless it actually has the highest score in the table (i.e. it IS Row 1). If the named county appears lower in the results, state its rank explicitly (e.g. "Harris County ranks 4th with a score of 99.93") and note that Row 1 has a higher score. NEVER describe a lower numeric score as higher than a higher numeric score.
...
IMPORTANT: FORBIDDEN geographic summary words — do NOT use "all", "only", "entirely", "dominate", "dominates", "dominated", "concentrated", or "exclusively" in any geographic description. Before stating ANY count (e.g. "8 of the top 10"), count the rows in the RESULTS table yourself — do not estimate. If even one row has a different state, you must acknowledge it. Use precise language: "6 of the top 10 counties are in California" not "California dominates the list." Never use outside knowledge about which states tend to be high-risk — base geographic observations solely on the state values present in the RESULTS rows."""
